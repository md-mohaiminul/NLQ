...
def dataset_gen_bert(data, vfeat_lens, tokenizer, max_pos_len, scope, num_workers=1):
    ...
    def worker(
        ...
        for record in tqdm(worker_data, total=len(worker_data), desc=description):
            ...
            #added later
            # if scope == 'train':
            #     if record["e_time"] - record["s_time"] < 3:
            #         mid = (record["s_time"] + record["e_time"])/2
            #         record["s_time"] = min(0, mid - 1.5)
            #         record["e_time"] = max(mid + 1.5, record["duration"])
            #         print(scope, record["s_time"], record["e_time"], record["duration"])

            word_ids = tokenizer([record["query"]])

            #added for clip
            word_ids = word_ids[0].tolist()
...
def gen_or_load_dataset(configs):
    ...
    feature_dir = os.path.join("/playpen-storage/mmiemon/ego4d/data/v1", configs.fv)
    ...
    if configs.predictor == "bert" or configs.predictor == 'clip':
        if configs.predictor == "bert":
            ...
            import clip
            clip_extractor, _ = clip.load("ViT-L/14", jit=False)
