...
def visual_feature_sampling(visual_feature, max_num_clips, sampling_strategy='mean'):
    ...
    for i in range(max_num_clips):
        ...
        if s_idx < e_idx:
            if sampling_strategy == 'mean':
                new_visual_feature.append(np.mean(visual_feature[s_idx:e_idx+1], axis=0))
            elif sampling_strategy == 'random':
                # sampling_index = random.randrange(s_idx, e_idx)
                # new_visual_feature.append(visual_feature[sampling_index])
                num_samples = max(2, int((e_idx-s_idx+1)*0.3))
                indices = random.sample(range(s_idx, e_idx), num_samples)
                indices.sort()
                new_visual_feature.append(np.mean(visual_feature[indices], axis=0))
            elif sampling_strategy == 'uniform':
                sampling_index = int((s_idx+e_idx-1)/2)
                new_visual_feature.append(visual_feature[sampling_index])
        else:
