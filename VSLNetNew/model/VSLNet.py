...
def masked_agerage_pooling(features, mask):
    mask = mask.unsqueeze(-1).expand(features.size())
    return torch.sum(features, dim=1)/torch.sum(mask, dim=1)
...
class VSLNet(nn.Module):
    def __init__(self, configs, word_vectors):
        ...
        #added later
        # self.video_mlp = nn.Sequential(LinearLayer(configs.dim, configs.dim, layer_norm=True, dropout=configs.drop_rate))
        #
        # self.query_mlp = nn.Sequential(LinearLayer(configs.dim, configs.dim, layer_norm=True, dropout=configs.drop_rate))
        ...
        #added later
        self.query_proj_nce = nn.Linear(configs.dim, 64)
        self.video_proj_nce = nn.Linear(configs.dim, 64)
        ...
        if configs.predictor == "bert":
            ...
            if configs.bert_type == 'base':
                self.query_affine = nn.Linear(768, configs.dim)
            elif configs.bert_type == 'large':
                self.query_affine = nn.Linear(1024, configs.dim)
            # init parameters
            self.init_parameters()
            self.embedding_net = BertEmbedding(configs.text_agnostic, bert_type= configs.bert_type)
            ...
            # init parameters
            self.init_parameters()
            self.embedding_net = ClipEmbedding(configs.text_agnostic, bert_type= configs.bert_type)
    ...
    def forward(self, word_ids, char_ids, video_features, v_mask, q_mask, s_labels=None, e_labels=None):
        ...
        # query_features = self.query_mlp(query_features)
        ...
        if s_labels is not None:
            mean_query = masked_agerage_pooling(query_features, q_mask)
            mean_video = []
            ...
            mean_video = self.video_proj_nce(mean_video)
            nce_loss = self.nce_loss(mean_query, mean_video)

            return h_score, start_logits, end_logits, nce_loss
