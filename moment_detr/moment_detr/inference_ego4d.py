...
import json
import copy
from evaluate_ego4d_nlq import evaluate_nlq_performance
...
def process_ego4d_submission(submission, topK, thresholds):
    #added later
    predictions = []
    #print(submission)
    for record in submission:
        timewindow_predictions = []
        for start_time, end_time, _ in record["pred_relevant_windows"]:
    ...
    with open('/playpen-storage/mmiemon/ego4d/data/annotations/nlq_val.json', "r") as file_id:
    ...
    for i in range(len(thresholds)):
        for j in range(len(topK)):
            result_dict['IoU' + str(thresholds[i]) + '-R@' + str(topK[j])] = results[i][j]
...
def eval_epoch_post_processing(submission, opt, gt_data, save_submission_filename):
    ...
    if opt.eval_split_name in ["val", "test"]:  # since test_public has no GT
        ...
        # )
        metrics = process_ego4d_submission(submission, topK=[1, 5, 10, 20], thresholds=[0.1, 0.3, 0.5])
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        if opt.span_loss_type == "l1":
            ...
            # for i in range (outputs['pred_spans'].shape[0]):
            #     q = query_meta[i]['query']
            #     with torch.no_grad():
            #         q = clip.tokenize(q).cuda()
            #         q = clip_model.encode_text(q)[0].float()
            #         q /= q.norm(dim=-1, keepdim=True)
            #     for j in range (outputs['pred_spans'].shape[1]):
            #         s, e = span_cxw_to_xx(outputs['pred_spans'][i][j])
            #         s = min(model_inputs['src_vid'].shape[1] -1, math.floor(s* model_inputs['src_vid'].shape[1]))
            #         e = min(model_inputs['src_vid'].shape[1] -1, math.ceil(e* model_inputs['src_vid'].shape[1]))
            #         v = model_inputs['src_vid'][i,s:(e+1),:768]
            #         v = torch.mean(v, dim=0)
            #         scores[i][j] = torch.matmul(q, v)
        ...
        for idx, (meta, spans, score) in enumerate(zip(query_meta, pred_spans.cpu(), scores.cpu())):
            ...
            # timewindow_predictions = []
            # for start_time, end_time, _ in cur_ranked_preds:
            #     timewindow_predictions.append([float(start_time), float(end_time)])
            # cur_query_pred = {
            #     "clip_uid": meta["vid"],
            #     "annotation_uid": meta["qid"].split('_')[0],
            #     "query_idx": int(meta["qid"].split('_')[1]),
            #     "predicted_times": copy.deepcopy(timewindow_predictions),
            # }
