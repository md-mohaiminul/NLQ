...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument("--init_model", default='ckpts/ViT_B_16/pytorch_model.bin.2', type=str, required=False, help="Initial model.")
...
def rerank_predictions(args, model, predictions, device, n_gpu, tokenizer, rawVideoExtractor):
    ...
    for cnt, pred_datum in enumerate(predictions):
        ...
        print(cnt, 'sequence_output', sequence_output.shape)
        ...
        for j in range(5):
            ...
            for i in range(j*10, (j+1)*10):
                ...
                e = min(math.ceil(pred_datum['predicted_times'][i][1]), s + 50)
            ...
            video_mask_batch = video_mask_batch.view(-1, video_mask_batch.shape[-1]) #(50, 12)
            #print(video_batch.shape, video_mask_batch.shape)  # (50, 1, 12, 1, 3, 224, 224) (50, 1, 12)
            ...
            print(cnt, 'visual_output', j, visual_output.shape, video_mask_batch.shape)
        ...
        pred_datum['predicted_times'][:50] = pred_datum['predicted_times'][:50][idx]
...
def main():
    ...
    print(predictions[0]['predicted_times'][:50])
    ...
    print(predictions[0]['predicted_times'][:50])
    result_save_path = '../VSLNet/checkpoints/nlq_official_clip_10s/vslnet_nlq_official_clip_10s_video_swin_512_bert' \
                       '/model/vslnet_29455_test_result_clip4clip.json'
...
# [[70.3125, 74.0625], [73.125, 74.0625], [72.1875, 75.0], [70.3125, 180.0], [70.3125, 82.5], [70.3125, 431.25], [177.1875, 180.0], [69.375, 72.1875], [72.1875, 73.125], [71.25, 72.1875], [70.3125, 71.25], [172.5, 180.0], [427.5, 431.25], [178.125, 179.0625], [80.625, 82.5], [177.1875, 195.9375], [70.3125, 99.375], [179.0625, 180.0], [80.625, 86.25], [75.0, 75.9375], [172.5, 174.375], [0.0, 74.0625], [178.125, 181.875], [177.1875, 178.125], [74.0625, 75.0], [73.125, 76.875], [82.5, 83.4375], [67.5, 75.9375], [173.4375, 175.3125], [60.9375, 74.0625], [195.0, 195.9375], [69.375, 70.3125], [429.375, 432.1875], [430.3125, 431.25], [77.8125, 82.5], [176.25, 184.6875], [404.0625, 431.25], [195.0, 199.6875], [180.0, 180.9375], [80.625, 122.8125], [428.4375, 429.375], [79.6875, 81.5625], [195.9375, 196.875], [171.5625, 176.25], [193.125, 195.9375], [121.875, 174.375], [427.5, 428.4375], [177.1875, 293.4375], [175.3125, 176.25], [83.4375, 85.3125]]
