...
def get_args(description='CLIP4Clip on Retrieval Task'):
    ...
    parser.add_argument('--feature_framerate', type=int, default=3, help='')
...
def rerank_predictions(args, model, predictions, device, n_gpu, tokenizer, rawVideoExtractor):
    ...
    for cnt, pred_datum in enumerate(predictions):
        ...
        print(cnt, pred_datum['predicted_times'][:10])
        # with open(f'outputs/vslnet_predictions_clip4clip_nms_fps3/{args.start_pred_index + cnt}.json', "w") as file_id:
...
def main():
    ...
    result_save_path = f'outputs/vslnet_predictions_clip4clip_nms_fps3/{args.start_pred_index}_{args.start_pred_index+399}.json'
